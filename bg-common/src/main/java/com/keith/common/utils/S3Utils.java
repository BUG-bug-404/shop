...
public class S3Utils {
	...
	private void init() {
		if (m_client == null) {
			/*m_bucketName = ProbaseUtils.getInstance().get(
					"s3.m_bucketName");
			String m_endpointurl = ProbaseUtils.getInstance().get(
					"s3.m_endpointurl");
			String accessKey = ProbaseUtils.getInstance().get("s3.accessKey");
			String secrectKey = ProbaseUtils.getInstance().get("s3.secrectKey");
			ClientConfiguration config = new ClientConfiguration();
			m_client = new OSSClient(m_endpointurl, accessKey, secrectKey,
					config);*/
			m_bucketName = "zhikelive";
			String m_endpointurl = "https://oss-cn-hangzhou.aliyuncs.com";
			String accessKey = "LTAIRcrZCBL6b7Zv";
			String secrectKey = "fEL4pcOqoCM3Zs69cvGo1tMTGAmyEI";
			ClientConfiguration config = new ClientConfiguration();
			m_client = new OSSClient(m_endpointurl, accessKey, secrectKey,
					config);
		}
