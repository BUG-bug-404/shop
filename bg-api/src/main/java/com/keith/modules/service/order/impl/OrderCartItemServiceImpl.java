...
public class OrderCartItemServiceImpl extends ServiceImpl<OrderCartItemDao, OrderCartItem> implements OrderCartItemService {
     ...
     */
    @Override
    public boolean add(Long productId, Long skuId, Integer count, Long userMemberId,int type) {
        /*判断数量是否大于库存*/
        /*判断此规格和商品id和用户id*/
    ...
    public PageUtils findAll(Integer currentPage, Integer pageSize, Long userMemberId, Integer status) {
        if (status != null && status == 0) {
            ...
            List<OrderCartItem> records = page.getRecords();
            if(records != null){
                records.stream().forEach(record->{
                    Long productSkuId = record.getProductSkuId();
                    ProSkuStock proSkuStock = proSkuStockService.getById(productSkuId);
                    record.setSp2(proSkuStock.getSp2());
                });
            }
